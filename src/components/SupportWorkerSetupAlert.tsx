...
export function SupportWorkerSetupAlert() {
  ...
  const profileItems: ProfileItem[] = useMemo(() => {
    ...
    return [
      {
        label: "Add Bio & Address",
        isComplete: !!(user.bio?.trim() && user.address?.trim()),
        key: "bio",
      },
      {
        label: "Add languages",
        isComplete: !!(user.languages && user.languages.length > 0),
        key: "languages",
      },
      {
      ...
      },
      {
        ...
        isComplete: !!(
          ...
          user.availability.weekdays.some(day => day.available && day.slots && day.slots.length > 0)
        ),
        key: "availability",
      },
      {
        label: "Add service locations",
        isComplete: !!(user.serviceAreaIds && user.serviceAreaIds.length > 0),
        key: "serviceAreas",
      },
      {
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 md:p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-gray-600 text-sm md:text-base">
              Provide your details and required documents to begin working under NDIS
            </p>
          </div>
          <Button
            onClick={() => navigate("/support-worker/setup")}
            className="bg-primary hover:bg-primary/90 text-white font-montserrat-semibold px-6 py-2.5 rounded-lg flex items-center justify-center gap-2 whitespace-nowrap transition-colors"
          >
            Setup Profile
