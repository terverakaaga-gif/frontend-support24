...
export function ProfileSetupAlert() {
  ...
  const profileItems: ProfileItem[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        isComplete: user.availability?.weekdays && user.availability.weekdays.length > 0,
        key: "availability"
      },
      {
        label: "Add compliance check",
        isComplete: user.verificationStatus?.identityVerified && 
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 space-y-4 fixed z-50 top-4 left-1/2 transform -translate-x-1/2">
      {/* white overlay */}
      <div className="absolute inset-0 bg-white/90 z-0 w-full h-full backdrop:blur"></div>
      ...
      <div className="bg-primary/10 border border-primary-100 rounded-lg p-4 md:p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Button onClick={() => navigate("/support-worker-setup")} className="bg-primary-600 hover:bg-primary-700 text-white font-medium px-6 py-2.5 rounded-lg flex items-center justify-center gap-2 whitespace-nowrap transition-colors">
          Setup Profile
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 md:p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-gray-900 font-semibold text-base md:text-lg">
            Complete Profile{" "}
            <span className="text-gray-500 font-normal">{completionPercentage}%</span>
          </h3>
          {incompleteCount > 0 && (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm md:text-base ${
                  item.isComplete ? 'text-gray-500 line-through' : 'text-gray-700'
                }`}>
                  {item.label}
                </span>
              </div>
