...
export default function ShiftDetailsDialog({
  ...
  const getStatusLabel = (s: string) =>
    s.replace(/_/g, " ").charAt(0).toUpperCase() + s.slice(1);

  // User Assignment Logic
  const userAssignment =
    shift && currentUserId
      ? shift.isMultiWorkerShift
        ? shift.workerAssignments?.find(
            (wa) =>
              (typeof wa.workerId === "object"
                ? wa.workerId._id
                : wa.workerId) === currentUserId
          )
        : shift.workerId &&
          (typeof shift.workerId === "object"
            ? shift.workerId._id
            : shift.workerId) === currentUserId
        ? { status: shift.status }
        : null
      : null;
  ...
  const handleReject = () => {
    if (!shift || !rejectReason.trim())
      return toast.error("Decline reason is required");
    acceptShiftMutation.mutate(
      { shiftId: shift._id, accept: false, declineReason: rejectReason },
      {
        onSuccess: () => {
  ...
  const handleStart = () =>
    ...
    updateStatusMutation.mutate(
      { shiftId: shift._id, status: "inProgress" },
  ...
  const handleComplete = () =>
    shift &&
    updateStatusMutation.mutate(
      { shiftId: shift._id, status: "completed" },
      { onSuccess: () => toast.success("Shift completed") }
    );
  ...
  const handleCancel = () => {
    ...
    cancelShiftMutation.mutate(
      { shiftId: shift._id, reason: rejectReason },
      {
        onSuccess: () => {
  ...
  const handleTimesheetSubmit = () => {
    ...
    const payload = {
      ...
      expenses: timesheetFormData.expenses.length
        ? timesheetFormData.expenses
        : undefined,
    };
  ...
  const handleTimesheetChange = (field: string, val: any) =>
    setTimesheetFormData((p) => ({ ...p, [field]: val }));

  const handleDurationSelect = (hours: number) => {
    if (!shift) return;
    setSelectedDuration(hours);
    const start = new Date(
      timesheetFormData.actualStartTime || shift.startTime
    );
    const end = new Date(
      start.getTime() +
  ...
  const handleTimesheetNestedChange = (
    index: number,
    field: string,
    value: any
  ) => {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-gray-100 rounded-lg max-w-2xl w-[95vw] max-h-[90vh] overflow-y-auto p-4 sm:p-10 z-50">
          ...
          <div className="space-y-4 mt-6">
            ...
            {/* Action Buttons - Update to pass shiftStartTime and use correct handlers */}
                ...
                <div className="bg-white shadow-sm border rounded-lg p-4">
                  {!showTimesheetForm ? (
                    <Button
                      ...
                      className="w-full"
                    >
                      <Document className="w-4 h-4 mr-2" /> Create Timesheet
                    </Button>
                  ) : (
                    <TimesheetForm
                      formData={timesheetFormData}
                      onChange={handleTimesheetChange}
                      onNestedChange={handleTimesheetNestedChange} // Use the proper handler
                      onAddExpense={() =>
                        setTimesheetFormData((p) => ({
                          ...p,
                          expenses: [
                            ...p.expenses,
                            {
                      ...
                      onToggleCustomTimes={(v) => {
                        setUseCustomTimes(v);
                        if (!v) setSelectedDuration(0);
                      }}
                      selectedDuration={selectedDuration}
                      onDurationSelect={handleDurationSelect}
                      minTime=""
                      maxTime="" // set constraints if needed
                      formatTime={formatTime}
