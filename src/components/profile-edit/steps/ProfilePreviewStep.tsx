...
export const ProfilePreviewStep: React.FC<ProfilePreviewStepProps> = ({ formData, isParticipant }) => {
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">Review your current information before making changes</p>
        </div>
      </div>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-semibold">{formData.firstName || "Not set"}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Last Name</p>
              <p className="font-semibold">{formData.lastName || "Not set"}</p>
            </div>
            <div>
      ...
      <Card>
        ...
        <CardContent>
          <p className="font-semibold">{formData.address || "No address provided"}</p>
        </CardContent>
      </Card>
              ...
              <CardContent className="space-y-2">
                ...
                {formData.planManaged !== undefined && (
                  <div>
                    ...
                    <Badge variant={formData.planManaged ? "default" : "secondary"}>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div>
                  ...
                  <p className="font-semibold">{formData.emergencyContact.name || "Not set"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Relationship</p>
                  <p className="font-semibold">{formData.emergencyContact.relationship || "Not set"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Phone</p>
                  <p className="font-semibold">{formData.emergencyContact.phone || "Not set"}</p>
                </div>
              </CardContent>
            </Card>
          )}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
        <p className="text-sm text-blue-800">
          Click <span className="font-semibold">Next</span> to start editing your profile information step by step.
        </p>
      </div>
    </div>
  );
