...
const mockJobData = {
  ...
  description:
    "Looking for a compassionate support worker to assist with daily personal care activities...",
  location: "Sydney, NSW 2000",
  hourlyRate: "35",
  availability: "full-time",
  status: "open",
  experienceRequired: "2",
  skills: ["personalCare", "mobilityAssistance", "mealPrep"],
  requiredQualifications: ["certIII", "firstAid", "ndisCheck"],
  languages: ["English"],
  stateId: "",
  regionId: "",
  serviceAreaIds: [],
...
export default function ParticipantJobFormPage() {
  ...
  const [formData, setFormData] = useState<JobFormData>(
    isEditMode
      ? {
          ...
          experienceRequired: mockJobData.experienceRequired,
          skills: mockJobData.skills,
          requiredQualifications: mockJobData.requiredQualifications,
  ...
  // File upload state
  const [jobImage, setJobImage] = useState<UploadedFile[]>([]);
  ...
  const handleQualificationToggle = (qualId: string) => {
    setFormData((prev) => ({
      ...prev,
      requiredQualifications: prev.requiredQualifications.includes(qualId)
        ? prev.requiredQualifications.filter((q) => q !== qualId)
        : [...prev.requiredQualifications, qualId],
    }));
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.skills.length === 0) {
      newErrors.skills = "Select at least one skill/service required";
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const imageFile = jobImage.length > 0 ? jobImage[0].file : null;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-6">
      ...
      >
        ...
        <div className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
          <div className="flex items-center gap-2 mb-4">
            ...
            <Label className="text-sm font-semibold text-gray-900">
              Service Location (Where support workers can apply from)
            </Label>
          </div>

          <p className="text-xs text-gray-500 mb-4">
            Select the location to target support workers in specific areas.
            Workers in these areas will see your job posting.
          </p>
        ...
        {/* Required Qualifications & Certifications - Two column layout */}
        <div className="mb-6">
          <Label className="text-sm font-semibold text-gray-700 mb-3 block">
            Required Qualifications & Certifications (Optional)
          </Label>
          <p className="text-xs text-gray-500 mb-3">
            Select the qualifications you require from support workers
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {qualificationOptions.map((qual) => {
              const isSelected = formData.requiredQualifications.includes(
                qual.id
              );
              return (
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    <Checkbox
                      ...
                      className={isSelected ? "border-white" : ""}
                    />
                    <span className="text-sm font-medium">{qual.label}</span>
                  </div>
                </div>
              );
            })}
