...
import supportWorkerService from "@/api/services/supportWorkerService";
  ...
  export default function SupportWorkerOrganizationDetailsPage() {
    ...
    const workers = useMemo(async ()=> {
      const promises = await Promise.allSettled(filteredWorkers.map(async (worker) => {
        const details = await supportWorkerService.getById(worker.workerId as string);
                            ...
                            <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500">
                                  Joined:{" "}
                                  {format(parseISO(worker.joinedDate), "dd/MM/yyyy")}
                                </p>
                              </div>
                            </div>
                ...
                <Card className="border-0 shadow-sm sticky top-6">
                  <CardContent className="p-0">
                    ...
                    <div className="p-3 md:p-4 space-y-3 md:space-y-4">
                      ...
                      <Button className="w-full bg-primary-600 hover:bg-primary-600 text-white font-montserrat-semibold text-sm md:text-base min-h-[44px]">
                        Remove Worker
                      </Button>
                    </div>
                  </CardContent>
              ...
              <div>
                ...
                <div className="space-y-2 md:space-y-3 text-xs md:text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-600">Joined Date:</span>
                    <span className="font-montserrat-semibold text-gray-900">
                      {format(parseISO(selectedWorker.joinedDate), "dd/MM/yyyy")}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Status:</span>
